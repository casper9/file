...
// ---------- repeat ----------
function buildScheduleSpec(datetimeISO, repeatType, intervalMinutes) {
  ...
  if (repeatType === "daily") return { kind: "cron", value: `${minute} ${hour} * * *` };
  if (repeatType === "weekly") return { kind: "cron", value: `${minute} ${hour} * * ${dow}` };
  if (repeatType === "monthly") return { kind: "cron", value: `${minute} ${hour} ${dom} * *` };
  if (repeatType === "interval_minutes") {
    const n = Number(intervalMinutes);
    const step = Number.isFinite(n) && n >= 1 ? Math.floor(n) : 1;
    return { kind: "cron", value: `*/${step} * * * *` };
  }
...
function updateRecent(accountId, targetsText, defaultMessage) {
  ...
  // targets: store each non-empty trimmed line (as user typed)
...
Schedule item:
{
  ...
  targetsText,              // keep raw user input for edit+recent
  defaultMessage,
  datetimeISO,
...
app.post("/accounts/:accountId/messages", (req, res) => {
  ...
  const {
    ...
    windowStart, windowEnd,
    gapSeconds,
    randomDelayMinSeconds, randomDelayMaxSeconds,
  } = req.body;
  ...
  const allowed = new Set(["once", "daily", "weekly", "monthly", "interval_minutes"]);
  ...
  if (rt === "interval_minutes") {
    const n = Number(intervalMinutes);
    if (!Number.isFinite(n) || n < 1) return res.status(400).json({ error: "intervalMinutes must be >= 1" });
...
app.put("/accounts/:accountId/messages/:id", (req, res) => {
  ...
  if (patch.repeatType !== undefined) {
    ...
    if (!allowed.has(rt)) return res.status(400).json({ error: "repeatType invalid" });
    cur.repeatType = rt;
    if (rt !== "interval_minutes") cur.intervalMinutes = undefined;
  }
...
app.post("/accounts/:accountId/logout", async (req, res) => {
  ...
  try {
    ...
    rimraf(accountSessionDir(accountId), () => {});
